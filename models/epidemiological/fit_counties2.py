	...
	if extrapolate > 0 :
		...
		if quick:
			for i in range(samples):
				...
				death_series = np.concatenate((data["deaths"].values[0:len(data)+start], death_series))
...
def submission(end, weight=True, guesses=None, start=-1, quick=False, moving=False):
	...
	if len(nonconvergent) > 0:
		...
		county_death_errors = np.concatenate((counties_death_errors, counties_death_errors2), axis=1)
		counties_fips = np.concatenate((counties_fips, counties_fips2))
